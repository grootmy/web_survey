...
type SlideInput = {
  ...
  cta: string;
  ctaColor: string;
  icon: "campaign" | "spa" | "rate_review" | "event";
  href: string;
};

export default function Carousel({ featuredNotice }: { featuredNotice?: Notice }) {
  ...
  const slides: SlideInput[] = [
    featuredNotice
      ? {
          ...
          title: "공지사항",
          subtitle: featuredNotice.title,
          description: featuredNotice.summary,
          cta: "공지 자세히 보기",
      ...
      : {
          ...
          description: "모두가 안전하게 이용할 수 있도록 공지를 꼭 확인해주세요.",
          cta: "공지 전체 보기",
          ctaColor: "text-blue-600",
          icon: "campaign",
          href: "/notices",
        },
    {
      ...
      description: "전문가가 추천하는 집에서 쉽게 따라할 수 있는 두피 관리 팁을 확인하세요.",
      cta: "꿀팁 보러가기",
      ctaColor: "text-green-600",
      icon: "spa",
      href: "/boards/manage-reviews",
    },
    {
      bg: "from-purple-500 to-indigo-500",
      title: "화제의 제품 리뷰",
      subtitle: "탈모 샴푸 A, 정말 효과 있을까?",
      description: "사용자들의 솔직한 후기와 평점으로 화제가 된 제품 A를 파헤쳐봅니다.",
      cta: "리뷰 전체 보기",
      ctaColor: "text-purple-600",
      icon: "rate_review",
      href: "/boards/product-reviews",
    },
    {
      bg: "from-yellow-400 to-orange-500",
      title: "커뮤니티 이벤트",
      subtitle: "나만의 관리 후기 공유 이벤트!",
      description: "정성스러운 후기를 남겨주신 분들 중 추첨을 통해 다양한 선물을 드립니다.",
      cta: "이벤트 참여하기",
...
function Slide({ bg, title, subtitle, description, cta, icon, ctaColor, href }: SlideInput) {
  const renderIcon = () => {
    switch (icon) {
      case 'campaign':
  ...
  return (
    <div className="min-w-full">
      <a className={`block bg-gradient-to-r ${bg} text-white p-8 md:p-12 relative`} href={href}>
