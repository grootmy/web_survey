...
export function transformPost(post: PostViewDto): Post {
  ...
  const rawCommentsCount =
    ...
    (post as any).answersCount;
  const computedCommentsCount =
    typeof rawCommentsCount === 'number'
      ? Number(rawCommentsCount)
  ...
  return {
    ...
    comments_count: computedCommentsCount,
    commentCount: computedCommentsCount,
    answersCount: computedCommentsCount,
    liked: false,
    disliked: false,
    imageUrls: images,
...
export async function fetchNotice(slug: string): Promise<Notice | null> {
  const base = APP_BASE_URL || '';
  const res = await fetch(`${base}/api/notices/${slug}`, {
    cache: 'no-store',
  });
  if (!res.ok) return null;
  const json = await res.json();
  return (json?.data as Notice) ?? null;
}
